...
class HybridCaptchaDetector {
    ...
    addUI() {
        ...
        toggleButton.title = `Captcha Solver: ${this.isEnabled ? 'Activo' : 'Inactivo'} (${this.useAPI ? 'Modelo Real' : 'Simple'})`;
        toggleButton.style.cssText = `
            ...
            width: 50px;
            height: 50px;
            background: ${this.isEnabled ? (this.useAPI ? '#28a745' : '#17a2b8') : '#6c757d'};
            color: white;
            border-radius: 50%;
            display: flex;
            align-items: center;
        ...
        toggleButton.addEventListener('click', () => {
            ...
            toggleButton.innerHTML = this.isEnabled ? '🤖' : '😴';
            toggleButton.title = `Captcha Solver: ${this.isEnabled ? 'Activo' : 'Inactivo'} (${this.useAPI ? 'Modelo Real' : 'Simple'})`;
            toggleButton.style.background = this.isEnabled ? (this.useAPI ? '#28a745' : '#17a2b8') : '#6c757d';
        });
        ...
        // Botón para cambiar entre API y Simple
        const switchButton = document.createElement('div');
        switchButton.innerHTML = this.useAPI ? '🧠' : '🎲';
        switchButton.title = `Modo: ${this.useAPI ? 'Modelo Real (API)' : 'Solver Simple'}`;
        switchButton.style.cssText = `
            ...
            width: 50px;
            height: 50px;
            background: ${this.useAPI ? '#28a745' : '#17a2b8'};
            color: white;
            border-radius: 50%;
            display: flex;
            align-items: center;
        ...
        switchButton.addEventListener('click', async () => {
            await this.toggleAPIMode();
            switchButton.innerHTML = this.useAPI ? '🧠' : '🎲';
            switchButton.title = `Modo: ${this.useAPI ? 'Modelo Real (API)' : 'Solver Simple'}`;
            switchButton.style.background = this.useAPI ? '#28a745' : '#17a2b8';
            toggleButton.style.background = this.isEnabled ? (this.useAPI ? '#28a745' : '#17a2b8') : '#6c757d';
            toggleButton.title = `Captcha Solver: ${this.isEnabled ? 'Activo' : 'Inactivo'} (${this.useAPI ? 'Modelo Real' : 'Simple'})`;
        });
